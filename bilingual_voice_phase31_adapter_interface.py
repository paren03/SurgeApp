"""Phase 31 - Multi-Adapter Interface.

Defines the Phase 31 callable adapter surface. Only two metadata-only
adapters are permitted: dummy_metadata_adapter and
...
_PHASE = "phase31.callable_interface.v1"
...
def get_phase31_callable_adapter_schema() -> dict[str, Any]:
    return {
        "version": _PHASE,
        ...
        "notes": [
            "Phase 31 permits exactly two metadata-only adapters.",
...
def get_phase31_allowed_adapter_types() -> list[str]:
    return list(ALLOWED_ADAPTER_TYPES)


def create_phase31_adapter_descriptor(
    adapter_name: str,
    adapter_type: str,
    test_only: bool = True,
) -> dict[str, Any]:
    return {
        ...
        "phase": _PHASE,
        "notes": ("phase31 metadata-only adapter descriptor; "
                  "in-process only"),
    }


def validate_phase31_adapter_descriptor(
    descriptor: Any,
) -> dict[str, Any]:
    reasons: list[str] = []
...
def create_phase31_adapter_request(
    ...
    return {
        ...
        "language_mode": (pkt.get("language_mode") or
                          (pkt.get("spoken_render_payload") or {})
                          .get("language_mode") or ""),
        "segment_count": (
            len((pkt.get("spoken_render_payload") or {})
                .get("segments") or [])
            if isinstance((pkt.get("spoken_render_payload") or {})
                          .get("segments"), list)
            else int((pkt.get("capability_summary") or {})
                      .get("segment_count", 0) or 0)),
        "safety_summary": pkt.get("safety_summary") or {},
        "spoken_render_payload": pkt.get("spoken_render_payload") or {},
        "phase": _PHASE,
        "notes": "phase31 multi-adapter request",
    }


def validate_phase31_adapter_request(request: Any) -> dict[str, Any]:
    reasons: list[str] = []
    ...
    for f in _REQUIRED_REQUEST_FIELDS:
        if f not in request:
            reasons.append(f"missing_field:{f}")
    dv = validate_phase31_adapter_descriptor(
...
def write_phase31_adapter_interface_report(
    report: dict[str, Any],
    output_path: str,
) -> str:
    p = Path(output_path)
...
__all__ = [
    "ALLOWED_ADAPTER_TYPES",
    "get_phase31_callable_adapter_schema",
    "get_phase31_allowed_adapter_types",
    "create_phase31_adapter_descriptor",
    "validate_phase31_adapter_descriptor",
    "create_phase31_adapter_request",
    "validate_phase31_adapter_request",
    "write_phase31_adapter_interface_report",
]
