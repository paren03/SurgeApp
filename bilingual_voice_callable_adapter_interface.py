"""Phase 30 - Callable Adapter Interface.

Defines the callable adapter surface without real audio execution.
Only the dummy_metadata_adapter is allowed in Phase 30.
"""
...
_PHASE = "phase30.callable_interface.v1"


ALLOWED_ADAPTER_TYPES = ("dummy_metadata_adapter",)
...
def get_callable_adapter_interface_schema() -> dict[str, Any]:
    return {
        "version": _PHASE,
        ...
        "notes": [
            "Phase 30 permits only dummy_metadata_adapter.",
...
def get_allowed_callable_adapter_types() -> list[str]:
    return list(ALLOWED_ADAPTER_TYPES)


def create_callable_adapter_descriptor(
    adapter_name: str,
    adapter_type: str = "dummy_metadata_adapter",
    test_only: bool = True,
) -> dict[str, Any]:
    return {
        "adapter_name": str(adapter_name or ""),
        "adapter_type": str(adapter_type or "dummy_metadata_adapter"),
        ...
        "phase": _PHASE,
        "notes": ("phase30 dummy metadata adapter descriptor; "
                  "no engine bound; in-process only"),
    }
...
def validate_callable_adapter_descriptor(
    descriptor: Any,
) -> dict[str, Any]:
    reasons: list[str] = []
...
def create_callable_adapter_request(
    ...
    return {
        ...
        "phase29_packet_id": pkt.get("packet_id") or pkt.get(
            "phase29_id") or "",
        ...
        "segment_count": int(
            (pkt.get("capability_summary") or {}).get(
                "segment_count", 0) or 0),
        "safety_summary": pkt.get("safety_summary") or {},
        "phase": _PHASE,
        "notes": "phase30 callable adapter request",
    }


def validate_callable_adapter_request(request: Any) -> dict[str, Any]:
    reasons: list[str] = []
    ...
    for f in _REQUIRED_REQUEST_FIELDS:
        if f not in request:
            reasons.append(f"missing_field:{f}")
    desc_val = validate_callable_adapter_descriptor(
        request.get("adapter_descriptor"))
    if not desc_val["ok"]:
        reasons.append("descriptor_invalid:" +
                       ",".join(desc_val["reasons"]))
...
def write_callable_adapter_interface_report(
    report: dict[str, Any],
    output_path: str,
) -> str:
    p = Path(output_path)
...
__all__ = [
    ...
    "get_callable_adapter_interface_schema",
    "get_allowed_callable_adapter_types",
    "create_callable_adapter_descriptor",
    "validate_callable_adapter_descriptor",
    "create_callable_adapter_request",
    "validate_callable_adapter_request",
    "write_callable_adapter_interface_report",
]
