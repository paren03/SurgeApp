...
_PHASE = "phase46.status_dashboard.v1"
...
def create_phase46_status_dashboard(
    ...
    return {
        "dashboard_id": f"p46dash_{int(time.time())}",
        "created_at": time.time(),
        "phase": _PHASE,
        "phase46_status":
            p.get("phase46_status", "no_packet"),
        "source_phase":
            p.get("source_phase", "phase45"),
        "archive_count":
            int(p.get("archive_count") or 0),
        "monotonic_ordering_status":
            p.get("monotonic_ordering_status",
                   "unknown"),
        "chain_integrity_status":
            p.get("chain_integrity_status", "unknown"),
        "manifest_verification_status":
            p.get("manifest_verification_status",
                   "unknown"),
        "long_horizon_verification_status":
            p.get("long_horizon_verification_status",
                   "unknown"),
        "tamper_suite_status": {
            "ok": bool(ts.get("ok")),
...
def validate_phase46_status_dashboard(
    dashboard: Any,
) -> dict[str, Any]:
    reasons: list[str] = []
    ...
    if dashboard.get("source_phase") != "phase45":
        reasons.append("source_phase_not_phase45")
...
def create_phase46_dashboard_markdown(
    dashboard: Any,
) -> str:
    if not isinstance(dashboard, dict):
        return ""
...
def write_phase46_status_dashboard(
...
def write_phase46_status_dashboard_markdown(
    markdown: str,
    output_path: str,
) -> str:
    p = Path(output_path)
...
__all__ = [
    "create_phase46_status_dashboard",
    "validate_phase46_status_dashboard",
    "create_phase46_dashboard_markdown",
    "write_phase46_status_dashboard",
    "write_phase46_status_dashboard_markdown",
]
