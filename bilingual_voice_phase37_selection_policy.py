"""Phase 37 - Four-Adapter Selection Policy."""
...
_PHASE = "phase37.selection_policy.v1"
...
def get_phase37_selection_policy() -> dict[str, Any]:
    return {
        "version": _PHASE,
        "allowed_adapter_types":
            list(p37i.ALLOWED_ADAPTER_TYPES),
        "rules": [
            ...
            "tracing is more important.",
            "High prosody density prefers "
            "prosody_density_metadata_adapter unless safety "
            "tracing is more important.",
            "Simple single-language payload may use "
            "dummy_metadata_adapter.",
            "Reject any adapter outside allowed four.",
        ],
        "notes": [
            "Selection never enables audio/TTS/subprocess.",
            "All four adapters return metadata only.",
        ],
    }


def reject_disallowed_phase37_adapter(
    ...
    if at not in p37i.ALLOWED_ADAPTER_TYPES:
...
def _safety_signal(request: dict[str, Any]) -> dict[str, Any]:
    """Detect rich safety / redaction signals on a payload."""
    ...
    merged = {**(safety if isinstance(safety, dict) else {}),
              **(spoken_safety if isinstance(spoken_safety, dict)
                 else {})}
    ...
    total_signal = (
        (5 if warn else 0) + replacements + risks +
...
def _prosody_signal(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(pros, dict):
        return {"total": 0}
    pause = sum(1 for k in pros if "pause" in str(k).lower() or
                 "break" in str(k).lower())
    emp = sum(1 for k in pros if "emphasis" in str(k).lower() or
               "stress" in str(k).lower() or
               "accent" in str(k).lower())
    tone = sum(1 for k in pros if "tone" in str(k).lower() or
...
def score_phase37_adapter_for_request(
    request: Any,
    adapter_descriptor: Any,
) -> dict[str, Any]:
    rej = reject_disallowed_phase37_adapter(adapter_descriptor)
    if rej["rejected"]:
    ...
    if at == "safety_redaction_trace_metadata_adapter":
        ...
        if safety["warn"] and safety["total_signal"] < 3:
            score += 0.1
            reasons.append("mild_safety_warning_no_redactions")
        ...
        if isinstance(segs, list) and len(segs) <= 1 \
                and pros["total"] == 0 and \
                safety["total_signal"] == 0:
            score += 0.05
            reasons.append("single_segment_payload")
        if safety["warn"] or pros["total"] >= 3 or \
                safety["total_signal"] >= 3:
            score -= 0.2
...
def choose_phase37_adapter(
    ...
    for d in pool:
        rej = reject_disallowed_phase37_adapter(d)
        if rej["rejected"]:
    ...
    if not safe_pool:
        return {
            ...
            "rejected": rejected, "candidate_adapters": [],
        ...
        for d in safe_pool:
            if d.get("adapter_type") == preferred_adapter or \
                ...
                return {
                    ...
                    "score_summary": {
                        d.get("adapter_name"):
                            score_phase37_adapter_for_request(
    ...
    scored = []
    for d in safe_pool:
        s = score_phase37_adapter_for_request(request, d)
...
def explain_phase37_selection(choice: Any) -> dict[str, Any]:
...
def write_phase37_selection_policy_report(
    report: dict[str, Any],
    output_path: str,
) -> str:
    p = Path(output_path)
...
__all__ = [
    "get_phase37_selection_policy",
    "score_phase37_adapter_for_request",
    "choose_phase37_adapter",
    "explain_phase37_selection",
    "reject_disallowed_phase37_adapter",
    "write_phase37_selection_policy_report",
]
