"""Phase 37 - Four-Adapter Interface.

Exactly four metadata-only adapters permitted:
...
_PHASE = "phase37.callable_interface.v1"
...
def get_phase37_callable_adapter_schema() -> dict[str, Any]:
    return {
        "version": _PHASE,
        ...
        "notes": [
            "Phase 37 permits exactly four metadata-only adapters.",
            "All execution-shape flags must be False.",
            "Any real adapter type must be rejected.",
            "Signed evidence + witness export + exchange are "
            "required by default for successful invocations.",
        ],
    }


def get_phase37_allowed_adapter_types() -> list[str]:
    return list(ALLOWED_ADAPTER_TYPES)


def create_phase37_adapter_descriptor(
    adapter_name: str,
    adapter_type: str,
    test_only: bool = True,
) -> dict[str, Any]:
    return {
        ...
        "phase": _PHASE,
        "notes": ("phase37 metadata-only adapter descriptor; "
                  "in-process only"),
    }


def validate_phase37_adapter_descriptor(
    descriptor: Any,
) -> dict[str, Any]:
    reasons: list[str] = []
...
def create_phase37_adapter_request(
    ...
    return {
        ...
        "phase": _PHASE,
        "notes": "phase37 four-adapter request",
    }


def validate_phase37_adapter_request(request: Any) -> dict[str, Any]:
    reasons: list[str] = []
    ...
    for f in _REQUIRED_REQUEST_FIELDS:
        if f not in request:
            reasons.append(f"missing_field:{f}")
    dv = validate_phase37_adapter_descriptor(
...
def write_phase37_adapter_interface_report(
    report: dict[str, Any],
    output_path: str,
) -> str:
    p = Path(output_path)
...
__all__ = [
    "ALLOWED_ADAPTER_TYPES",
    "get_phase37_callable_adapter_schema",
    "get_phase37_allowed_adapter_types",
    "create_phase37_adapter_descriptor",
    "validate_phase37_adapter_descriptor",
    "create_phase37_adapter_request",
    "validate_phase37_adapter_request",
    "write_phase37_adapter_interface_report",
]
