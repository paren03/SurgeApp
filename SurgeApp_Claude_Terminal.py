...
try:
    ...
    from PySide6.QtGui import QColor, QPainter, QPainterPath, QTextCursor, QDragEnterEvent, QDropEvent
...
# Continues-update (overnight unattended improvement loop)
NIGHTLY_MD_PATH = MEMORY_DIR / "nightly_updates.md"
NIGHTLY_JSONL_PATH = MEMORY_DIR / "nightly_updates.jsonl"
CONTINUES_STOP_FLAG = MEMORY_DIR / "continues_update.stop"
CONTINUES_STATE_PATH = MEMORY_DIR / "continues_update_state.json"

# Five canonical stages echoed to the main chat as short Luna lines.
_STAGE_HEADLINES = {
    "CLAIM":           (1, "Queued Aider patch"),
    "RUN_AIDER_START": (2, "Aider running"),
    "RUN_AIDER_END":   (2, "Aider returned"),
    "DIFF_SAVED":      (3, "Diff saved"),
    "VERIFY_COMPILE":  (4, "Verifying compile"),
    "VERIFY_IMPORT":   (4, "Verifying import"),
    "APPLY":           (5, "Applying patch"),
    "DONE":            (5, "Done"),
    "FAILED":          (5, "Failed"),
}
_CONTINUES_TRIGGERS = ("continues update", "/continuesupdate")
_CONTINUES_STOP_TRIGGERS = ("stop continues update", "/continuesupdate stop", "stop continuesupdate")
_UPDATES_QUERY_TRIGGERS = ("what are your updates", "what are the updates", "/updates")
...
if PYSIDE_AVAILABLE:
    ...
    class LunaClaudeStyleWindow(QWidget):
        ...
        def _on_send(self) -> None:
            ...
            # --- Continues-update (overnight loop) ------------------------------
            low_full = raw.lower().strip()
            if any(low_full == t or low_full.startswith(t + " ") for t in _CONTINUES_STOP_TRIGGERS):
                self.input.clear()
                self._stop_continues_update()
                return
            if any(low_full == t or low_full.startswith(t + " ") for t in _CONTINUES_TRIGGERS):
                self.input.clear()
                self._start_continues_update()
                return
            if any(low_full == t or low_full.startswith(t) for t in _UPDATES_QUERY_TRIGGERS):
                self.input.clear()
                self._show_nightly_updates_summary()
                return
        ...
        def _tick_tasks(self) -> None:
            try:
                if self.tabs.currentIndex() == 4:  # Tasks
                    self._refresh_tasks_list()
            except Exception:
                pass
        ...
        def _on_feed_event(self, row: dict) -> None:
            """
            Right Inspector: full live feed.
            Main chat: short Stage N/5 headline only for canonical bridge events.
            """
            try:
                ...
                # Mirror canonical bridge stages into main chat as short status lines.
                stage = _STAGE_HEADLINES.get(evt.upper())
                if stage is not None:
                    n, label = stage
                    task_id = str(row.get("task_id") or "")
                    suffix = f"  ({task_id})" if task_id else ""
                    self.chat.append_ai(f"Stage {n}/5: {label}{suffix}")
            except Exception:
                pass

        # ------------------------------
        # Continues-update controller (overnight loop)
        # ------------------------------
        def _resolve_python_for_worker(self) -> str:
            try:
                python_exe = Path(sys.executable)
                pythonw = python_exe.parent / "pythonw.exe"
                ...
                pass
            try:
                cand = Path(os.environ.get("LOCALAPPDATA", "")) / "Microsoft" / "WindowsApps" \
        ...
        def _start_continues_update(self) -> None:
            try:
                CONTINUES_STOP_FLAG.unlink(missing_ok=True)
            except Exception:
                pass
            try:
                exe = self._resolve_python_for_worker()
                subprocess.Popen(
                    [exe, str(WORKER_PATH), "--continues-update-start"],
                    cwd=str(PROJECT_DIR),
                    creationflags=CREATE_NO_WINDOW,
                    stdin=subprocess.DEVNULL, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL,
                    close_fds=True,
                )
                self.chat.append_ai("Continues-update loop started. I'll keep going until you say 'stop continues update'.")
                self.plan.append_block("Continues Update", "Loop started in background.", accent="#27c93f")
            except Exception as exc:
                self.chat.append_ai(f"Could not start continues-update loop: {exc}")
                self.plan.append_block("Continues Update", f"Start failed: {exc}", accent="#ff5f56")

        def _stop_continues_update(self) -> None:
            try:
                CONTINUES_STOP_FLAG.parent.mkdir(parents=True, exist_ok=True)
                CONTINUES_STOP_FLAG.write_text(_now_iso(), encoding="utf-8", errors="replace")
                self.chat.append_ai("Stopping continues-update loop. It will exit at the next checkpoint.")
                self.plan.append_block("Continues Update", "Stop flag written.", accent="#ffbd2e")
            except Exception as exc:
                self.chat.append_ai(f"Could not write stop flag: {exc}")

        def _show_nightly_updates_summary(self) -> None:
            """Read memory/nightly_updates.md and present a clean summary in the main chat."""
            try:
                if not NIGHTLY_MD_PATH.exists():
                    self.chat.append_ai("No nightly updates yet — I haven't run a continues-update cycle.")
                    return
                text = NIGHTLY_MD_PATH.read_text(encoding="utf-8", errors="replace").strip()
                if not text:
                    self.chat.append_ai("Nightly log exists but is empty.")
                    return
                # Take the last 5 cycle blocks (each block starts with '## Cycle').
                blocks = re.split(r"^## Cycle ", text, flags=re.MULTILINE)
                tail_blocks = blocks[-5:] if len(blocks) > 1 else blocks
                summary = "\n\n".join("## Cycle " + b.strip() for b in tail_blocks if b.strip() and not b.startswith("#"))
                if not summary:
                    summary = text[-4000:]
                self.chat.append_ai("Here's what I worked on overnight:\n\n" + summary[:6000])
                self.preview.clear()
                self.preview.append_block("Nightly updates (full)", text[:200000], accent="#a6a6ad")
            except Exception as exc:
                self.chat.append_ai(f"Could not read nightly updates: {exc}")
